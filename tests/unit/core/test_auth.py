...
class TestTestAuthClass:
    ...
    def test_session_token_expiry(self):
        ...
        # 模擬時間過期（不使用實際 sleep）
        with patch('time.time', return_value=time.time() + 2):  # 模擬 2 秒後
...
class TestAuthDecorator:
    ...
    def test_require_test_auth_simple_password(self, client):
        """測試簡單密碼認證裝飾器"""
        # 初始化認證
        ...
        # 使用 Flask 應用上下文和請求上下文
        with client.application.app_context():
            with client.application.test_request_context(method='GET'):
                # 模擬 Flask request 和 session
                with patch('src.core.auth.request') as mock_request, \
                     patch('src.core.auth.session') as mock_session, \
                     patch('src.core.auth.current_app') as mock_app:
    ...
    def test_require_test_auth_with_post_login(self, client):
        """測試 POST 登入請求"""
        config = {'auth': {'method': 'simple_password', 'password': 'test123'}}
        init_test_auth_with_config(config)
        ...
        with client.application.app_context():
            with client.application.test_request_context(method='POST'):
                with patch('src.core.auth.request') as mock_request, \
                     patch('src.core.auth.session') as mock_session, \
                     patch('src.core.auth.current_app') as mock_app:
                    ...
                    with patch('src.core.auth.test_auth.generate_login_form') as mock_form:
                        mock_form.return_value = "錯誤表單"  # 只返回表單內容，不包含狀態碼
                        result = protected_view()
                        assert result == ("錯誤表單", 401)  # 整體結果應該是 (表單內容, 401)
    
    def test_require_test_auth_basic_auth(self, client):
        """測試基本認證裝飾器"""
        config = {
            'auth': {
                'method': 'basic_auth',
    ...
    def test_require_test_auth_token(self, client):
        """測試 token 認證裝飾器"""
        config = {
            'auth': {
                'method': 'token',
    ...
    def test_testing_environment_bypass(self, client):
        """測試在測試環境中繞過認證"""
        config = {'auth': {'method': 'simple_password', 'password': 'test123'}}
        init_test_auth_with_config(config)
